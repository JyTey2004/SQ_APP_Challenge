const MerchantMembership = require('../models/MerchantMembership');

// Create a new membership
async function createMembership(req, res) {
    try {
        const membership = await MerchantMembership.create(req.body);
...
async function getMembershipById(req, res) {
    const { id } = req.params;
    try {
        const membership = await MerchantMembership.findById(id);
        if (!membership) {
...
async function updateMembership(req, res) {
    const { id } = req.params;
    try {
        const membership = await MerchantMembership.findByIdAndUpdate(id, req.body, {
            new: true,
          });
          if (!merchant) {
...
async function deleteMembership(req, res) {
    const { id } = req.params;
    try {
        const membership = await MerchantMembership.findByIdAndDelete(id);
        if (!membership) {
            return res.status(404).json({ error: 'Membership not found' });
        }
        res.json(membership);
